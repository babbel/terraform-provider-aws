...
func testAccAWSBudgetsBudgetConfigWithNotification_Basic(budget budgets.Budget, notifications []budgets.Notification, emails []string, topics []string) string {
	timePeriodStart := budget.TimePeriod.Start.Format("2006-01-02_15:04")
	timePeriodEnd := budget.TimePeriod.End.Format("2006-01-02_15:04")
	...
	notificationString := strings.Join(notificationStrings, "\n")
...
resource "aws_budgets_budget" "foo" {
	...
	`, *budget.BudgetName, *budget.BudgetType, *budget.BudgetLimit.Amount, *budget.BudgetLimit.Unit, *budget.CostTypes.IncludeTax, *budget.CostTypes.IncludeSubscription, *budget.CostTypes.UseBlended, timePeriodStart, timePeriodEnd, *budget.TimeUnit, notificationString)
}
...
func testAccAWSBudgetsBudgetConfigNotificationSnippet(notification budgets.Notification, emails []string, topics []string) string {
	subscriberEmailAddresses := fmt.Sprintf(`["%s"]`, strings.Join(emails, "\", \""))
	subscriberSnsTopicArns := fmt.Sprintf(`["%s"]`, strings.Join(topics, "\", \""))
	...
	`, *notification.Threshold, *notification.ThresholdType, *notification.NotificationType, subscriberEmailAddresses, subscriberSnsTopicArns, *notification.ComparisonOperator)
}
